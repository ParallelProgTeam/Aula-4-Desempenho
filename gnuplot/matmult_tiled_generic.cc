...
void mat_mult(int n, double **A, double **B, double **C) {
   ...
   int fim =  ((n + N_b - 1)/N_b) * N_b;
   printf("Fim %i\n", fim);
   for (int i = 0; i < fim; i+=N_b){
     for (int j = 0; j < fim; j+=N_b){
       for (int ii = 0; ii < N_b; ii++) {
         for (int jj = 0; jj < N_b; jj++) {
	   if ( (i + ii < n) && (j + jj < n)) 
             Cc[ii][jj] = C[i+ii][j+jj];
	   else 
             Cc[ii][jj] = 0;
	 }
       }
       for (int k = 0; k < fim; k+=N_b){
         for (int jj = 0; jj < N_b; jj++) {
           for (int kk = 0; kk < N_b; kk++){
	     if ((k + kk < n) && (j + jj < n)) {
               Aa[jj][kk] = A[i+jj][k+kk];
               Bb[jj][kk] = B[k+jj][j+kk];
	     }
	     else {
               Aa[jj][kk] = 0;
               Bb[jj][kk] = 0;
